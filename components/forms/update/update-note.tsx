...
import {  editNoteAction } from "@/server/actions/admin/note";
...
const UpdateNoteForm = ({ notePost }: EditNoteFormProps) => {
  // form states
  ...
  const form = useForm<z.infer<typeof noteZodSchema>>({
    ...
    defaultValues: {
      ...
      published: notePost.published,
      content: notePost.content.join(" "),
    },
  });
  //   async function create new note
  const onSubmit = (values: z.infer<typeof noteZodSchema>) => {
    startTransition(async () => {
      const result = await editNoteAction(notePost.id, values);
