        ...
        <div className="grid grid-cols-12 gap-4 overflow-hidden lg:pb-5 pb-2">
          {promotion.products.flatMap((product) => 
            product.images.map((image, imageIndex) => {
              ...
              const globalIndex = promotion.products.slice(0, promotion.products.indexOf(product)).reduce((acc, p) => acc + p.images.length, 0) + imageIndex;
                  ...
                  <Link href={`/product/${product.id}`} className="block h-full">
                    <div className="w-auto h-full relative overflow-hidden rounded-xl">
                      <Image
                        ...
                        height={600}
                        width={1200}
                        className="h-full w-full object-cover rounded-xl transition-all duration-500 group-hover:scale-110 group-hover:brightness-110 group-hover:contrast-105"
                      />
