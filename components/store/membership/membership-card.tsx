...
export function MembershipCard({ membershipInfo }: MembershipCardProps) {
  ...
  const nextTier = allTiers.find(
    (t) => user.membership && t.minPoints > user.membership.minPoints
  );
        ...
        <div className="py-4">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {allTiers
              .filter(
                (tier) =>
                  user.membership && tier.minPoints > user.membership.minPoints
              )
              .map((tier) => (
                <Card
                  key={tier.id}
                  className="flex flex-col relative pt-12 opacity-60"
                >
                  {tier.popular && (
                    <Badge className="flex space-x-2 tabular-nums absolute top-0 right-0 justify-center items-center translate-x-1 -translate-y-1">
                      Popular
                  ...
                  <CardHeader>
                    <CardTitle className="font-medium text-center text-muted-foreground">
                      {tier.title}
                    </CardTitle>
