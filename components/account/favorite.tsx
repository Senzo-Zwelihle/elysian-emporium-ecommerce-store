  ...
  return (
    <Card className="group hover:shadow-md transition-all duration-200 relative overflow-hidden">
      ...
      <Button
        variant="ghost"
        size="sm"
        className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity bg-background/80 backdrop-blur-sm hover:bg-destructive hover:text-destructive-foreground"
      >
        <XIcon />
      </Button>
...
export const FavoritesTab = ({ favorites }: FavoritesTabProps) => {
  ...
  const availableItems = favorites.filter(
    (fav) => fav.product.stock > 0
  ).length;
