              ...
              {tabs.map((tab) => {
                ...
                return (
                  <TabsTrigger
                    key={tab.id}
                    value={tab.id}
                    className="flex items-center justify-center gap-2 h-12 px-4 py-2 text-xs font-medium transition-all data-[state=active]:shadow-sm data-[state=active]:bg-background min-w-fit whitespace-nowrap"
                  >
            ...
            {tabs.map((tab) => {
              ...
              return (
                <TabsTrigger
                  key={tab.id}
                  value={tab.id}
                  className="flex items-center justify-center gap-2 h-12 px-3 py-2 text-sm font-medium transition-all data-[state=active]:shadow-sm data-[state=active]:bg-background"
                >
                  <Icon className="h-4 w-4 flex-shrink-0" />
                  ...
                  {tab.count !== undefined && tab.count > 0 && (
                    <Badge
                      variant="secondary"
                      className="ml-1 px-1.5 py-0.5 text-xs min-w-5 h-5"
                    >
                      {tab.count > 99 ? "99+" : tab.count}
                    </Badge>
                  )}
