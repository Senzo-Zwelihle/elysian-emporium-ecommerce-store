...
import { awardPoints, calculateOrderPoints, hasReceivedPointsFor, POINTS_CONFIG } from "@/lib/utils/points";
...
export async function awardReviewPointsAction(userId: string, productId: string) {
  try {
    ...
    if (alreadyAwarded) {
      return { success: false, message: "Points already awarded for this review" };
    }
